...
materials. It is included in the base R \pkg{datasets} package as
\code{iris}, making it easy for users to access without knowing much
...
groups with zero missing values make \code{iris} an inadequate and stale
dataset for teaching and learning modern data science skills. Users
would benefit from working with a more representative, real-world
...
first-listed citation for the dataset), inextricably linking \code{iris}
to eugenics research. Thus, a modern alternative to \code{iris} is
needed. In this paper, we introduce the \CRANpkg{palmerpenguins} R
package, which includes body size measurements collected from 2007 -
2009 for three species of \emph{Pygoscelis} penguins that breed on
islands throughout the Palmer Archipelago, Antarctica. The
\code{penguins} dataset in \pkg{palmerpenguins} provides an
approachable, charismatic, and near drop-in replacement for \code{iris}
with topical relevance for polar climate change and environmental
impacts on marine predators. Since the release on CRAN in July 2020, the
\pkg{palmerpenguins} package has been downloaded over 402,000 times,
highlighting the demand and widespread adoption of this viable
\code{iris} alternative. We directly compare the \code{iris} and
\code{penguins} datasets for selected analyses to demonstrate that R
users, in particular teachers and learners currently using \code{iris},
can switch to the Palmer Archipelago penguins for many use cases
including data wrangling, visualization, linear modeling, multivariate
analysis (e.g., PCA), cluster analysis and classification (e.g., by
k-means).
}
...
to as \code{iris}), including linear relationships and multivariate
...
modeling, multivariate analyses, and machine learning. The \code{iris}
dataset is built into a number of software packages including the
auto-installed \pkg{datasets} package in R \citep{r_core_team_r_2019},
...
\code{iris} remains ubiquitous in statistics, computational methods,
...
educators may want to move on from \code{iris}. First, the dataset lacks
...
\code{iris}, two (\emph{Sepal.Width} and \emph{Sepal.Length}) are not
...
\code{iris} contains equal sample sizes for each of the three species
...
factor (\emph{Species}) in \code{iris} limits options for analyses.
Finally, due to its publication in the \emph{Annals of Eugenics} by
statistician R.A. Fisher \citep{fisher_use_1936}, \code{iris} is
...
features of \code{iris} in data science and statistics materials, we
...
Here, we describe an alternative to \code{iris} that largely satisfies
...
\code{iris} for many use cases in statistics and data science education.
...
The data in the \pkg{palmerpenguins} R package are available for use by
...
available on the \pkg{palmerpenguins} package website:
...
The \pkg{palmerpenguins} R package contains two data objects:
\code{penguins\_raw} and \code{penguins}. The \code{penguins\_raw} data
...
(\code{penguins\_raw} properties are summarized in Appendix B). We
generally recommend using the curated data in \code{penguins}, which is
a subset of \code{penguins\_raw} retaining all 344 observations,
...
\tightlist
\item
  \code{species}: a factor denoting the penguin species (Adélie,
  Chinstrap, or Gentoo)
\item
  \code{island}: a factor denoting the Palmer Archipelago island in
  Antarctica where each penguin was observed (Biscoe Point, Dream
  Island, or Torgersen Island)
\item
  \code{bill\_length\_mm}: a number denoting length of the dorsal ridge
  of a penguin bill (millimeters)
\item
  \code{bill\_depth\_mm}: a number denoting the depth of a penguin bill
  (millimeters)
\item
  \code{flipper\_length\_mm}: an integer denoting the length of a
  penguin flipper (millimeters)
\item
  \code{body\_mass\_g}: an integer denoting the weight of a penguin's
  body (grams)
\item
  \code{sex}: a factor denoting the sex of a penguin sex (male, female)
  based on molecular data
\item
  \code{year}: an integer denoting the year of study (2007, 2008, or
  2009)
\end{itemize}

The same data exist as comma-separated value (CSV) files in the package
(\file{penguins\_raw.csv} and \file{penguins.csv}), and can be read in
using the built-in \code{path\_to\_file()} function in
\pkg{palmerpenguins}. For example,

will read in \file{penguins.csv} as if from an external file, thus
automatically parsing \code{species}, \code{island}, and \code{sex}
...
The \code{penguins} data in \pkg{palmerpenguins} is useful and
approachable for data science and statistics education, and is uniquely
well-suited to replace the \code{iris} dataset. Comparisons presented
are selected examples for common \code{iris} uses, and are not
exhaustive.
...
Both \code{iris} and \code{penguins} are in tidy format
...
dimensions (columns × rows) are 5 × 150 and 8 × 344 for \code{iris} and
\code{penguins}, respectively, and sample sizes within species are
similar (Tables 1 \& 2).

Notably, while sample sizes in \code{iris} across species are all the
same, sample sizes in \code{penguins} differ across the three species.
The inclusion of three factor variables in \code{penguins}
(\code{species}, \code{island}, and \code{sex}), along with \code{year},
create additional opportunities for grouping, faceting, and analysis
compared to the single factor (\code{Species}) in \code{iris}.

Unlike \code{iris}, which contains only complete cases, the
\code{penguins} dataset contains a small number of missing values
...
and create added learning opportunity to the \code{penguins} dataset.
...
measurements in \code{penguins} (flipper length, body mass, bill length
and depth; Figure \ref{fig:penguin-pairs}) and \code{iris} (sepal width
...
Both \code{penguins} and \code{iris} offer numerous opportunities to
...
scatterplot made with the \code{iris} dataset reveals a clear linear
relationship between petal length and petal width. Using \code{penguins}
...
all three species is approximately linear for both \code{iris} and
\code{penguins}. Teachers may encourage students to explore how simple
...
\ref{fig:penguin-pairs}) Simpson's Paradox example in \code{penguins}:
...
commonly used to explore patterns in multivariate data. The \code{iris}
...
measurements in \code{penguins} and \code{iris} (both normalized prior
...
\code{penguins}, 88.15\% of total variance is captured by the first two
PCs, compared to 95.81\% for \code{iris}, with a similarly large
...
machine learning and classification, and again, the \code{iris} dataset
is frequently used in introductory examples. The \code{penguins} data
...
each dataset: for \code{iris}, petal width and petal length, and for
\code{penguins}, bill length and bill depth. All variables are scaled
...
Archipelago \emph{Pygoscelis} penguins, available as \code{penguins} in
the \pkg{palmerpenguins} R package, offer a near drop-in replacement for
\code{iris} in a number of common use cases for data science and
...
teaching and learning opportunities in \code{penguins} are increased due
to a greater number of variables, missing values, unequal sample sizes,
and Simpson's Paradox examples. Importantly, the \code{penguins} dataset
...
the \code{penguins} dataset can facilitate discussions more broadly on
...
Data in the \code{penguins} object have been minimally updated from
\code{penguins\_raw} as follows:

\begin{itemize}
\tightlist
\item
  All variable names are converted to lower snake case (e.g.~from
  \code{Flipper Length (mm)} to \code{flipper\_length\_mm})
\item
  Entries in \code{species} are truncated to only include the common
...
\item
  ...
  updated to \code{NA}
\item
  \code{culmen\_length\_mm} and \code{culmen\_depth\_mm} variable names
  are updated to \code{bill\_length\_mm} and \code{bill\_depth\_mm},
  respectively
\item
  Class for categorical variables (\code{species}, \code{island},
  \code{sex}) is updated to factor
\item
...
TensorFlow Datasets. Information and examples for \code{penguins} data
