...
\textbf{palmerpenguins} R package \citep{R-palmerpenguins}, which
...
\textbf{palmerpenguins} provides an approachable, charismatic, and near
drop-in replacement for \textbf{iris} with topical relevance for polar
climate change and environmental impacts on marine predators. Since the
release on CRAN in July 2020, the \textbf{palmerpenguins} package has
been downloaded over 402,000 times, highlighting the demand and
widespread adoption of this viable \textbf{iris} alternative. We
directly compare the \textbf{iris} and \textbf{penguins} datasets for
selected analyses to demonstrate that R users, in particular teachers
and learners currently using \textbf{iris}, can switch to the Palmer
...
auto-installed datasets package in R \citep{R-base}, Python's
...
\item
  ...
  \citep{R-broom}: for pre-processing and statistical analyses
\item
...
\item
  \textbf{base} and \textbf{stats} \citep{R-base}: for various analyses
  throughout
\end{itemize}
