...
\code{penguinsraw} and \code{penguins}. The \code{penguinsraw} data
...
(\code{penguinsraw} properties are summarized in Appendix B). We
generally recommend using the curated data in \code{penguins}, which is
a subset of \code{penguinsraw} retaining all 344 observations, minimally
...
\item
  \code{bill_length_mm}: a number denoting length of the dorsal ridge of
  a penguin bill (millimeters)
\item
  \code{bill_depth_mm}: a number denoting the depth of a penguin bill
  (millimeters)
\item
  \code{flipper_length_mm}: an integer denoting the length of a penguin
  flipper (millimeters)
\item
  \code{body_mass_g}: an integer denoting the weight of a penguin's body
  (grams)
\item
...
(\file{penguins_raw.csv} and \file{penguins.csv}), and can be read in
using the built-in \code{path_to_file()} function in
...
\code{penguinsraw} as follows:

\begin{itemize}
\tightlist
\item
  All variable names are converted to lower snake case (e.g.~from
  \code{Flipper Length (mm)} to \code{flipper_length_mm})
\item
...
\item
  ...
  updated to \code{NA}
\item
  \code{culmen_length_mm} and \code{culmen_depth_mm} variable names are
  updated to \code{bill_length_mm} and \code{bill_depth_mm},
  respectively
\item
