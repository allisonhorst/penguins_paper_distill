...
The \pkg{palmerpenguins} R package contains two data objects: \code{penguins_raw} and \code{penguins}. The \code{penguins_raw} data consists of all raw data for `r ncol(palmerpenguins::penguins_raw)` variables, recorded completely or in part for `r nrow(palmerpenguins::penguins_raw)` individual penguins, accessed directly from EDI (\code{penguins_raw} properties are summarized in Appendix B). We generally recommend using the curated data in \code{penguins}, which is a subset of \code{penguins_raw} retaining all `r nrow(palmerpenguins::penguins)` observations, minimally updated (Appendix A) and reduced to the following eight variables:
...
-   \code{bill_length_mm}: a number denoting length of the dorsal ridge of a penguin bill (millimeters)
-   \code{bill_depth_mm}: a number denoting the depth of a penguin bill (millimeters)
-   \code{flipper_length_mm}: an integer denoting the length of a penguin flipper (millimeters)
-   \code{body_mass_g}: an integer denoting the weight of a penguin's body (grams)
...
The same data exist as comma-separated value (CSV) files in the package (\file{penguins_raw.csv} and \file{penguins.csv}), and can be read in using the built-in \code{path_to_file()} function in \pkg{palmerpenguins}. For example,
...
Data in the \code{penguins} object have been minimally updated from \code{penguins_raw} as follows:

-   All variable names are converted to lower snake case (e.g. from \code{Flipper Length (mm)} to \code{flipper_length_mm})
...
-   \code{culmen_length_mm} and \code{culmen_depth_mm} variable names are updated to \code{bill_length_mm} and \code{bill_depth_mm}, respectively
