...
Both `penguins` and `iris` offer numerous opportunities to explore linear relationships and correlations, within and across species (Figures `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:penguin-pairs-web)', '\\@ref(fig:penguin-pairs)'))` & `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:iris-pairs-web)', '\\@ref(fig:iris-pairs)'))`). A bivariate scatterplot made with the `iris` dataset reveals a clear linear relationship between petal length and petal width. Using `penguins` (Figure `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:linear-web)', '\\@ref(fig:linear-example)'))`), we can create a uniquely similar scatterplot with flipper length and body mass. The overall trend across all three species is approximately linear for both `iris` and `penguins`. Teachers may encourage students to explore how simple linear regression results and predictions differ when the species variable is omitted, compared to, for example, multiple linear regression with species included (Figure `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(fig:linear-web)', '\\@ref(fig:linear-example)'))`).
...
Python users can load the palmerpenguins datasets into their Python environment using the following code to install and access data in the [palmerpenguins Python package](https://pypi.org/project/palmerpenguins/):
...
Julia users can access the penguins data in the **PalmerPenguins.jl** package. Example code to import the penguins data through **PalmerPenguins.jl** (more information on **PalmerPenguins.jl** from David Widmann can be found [here](https://github.com/devmotion/PalmerPenguins.jl)):
...
TensorFlow users can access the penguins data in TensorFlow Datasets. Information and examples for **penguins** data in TensorFlow can be found [here](https://www.tensorflow.org/datasets/catalog/penguins).
